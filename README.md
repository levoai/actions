...
The action wraps common `shadownet scan` options (target auth, scan mode, crawl limits, AI, CVE, chatbot, domain filters, etc.). Timeout is passed as `--scan-timeout` via the `timeout` input. See [`dast-scan/action.yaml`](dast-scan/action.yaml) for all inputs. Use `extra-args` for any other CLI flags.
